...
abstract class Updater implements Standard_Service, Registerable, Plugin_Activation_Listener
{
    ...
    {
        return ['version_option_name' => $this->plugin->get_slug() . '_version', 'needs_update_db_notice' => ['title' => sprintf(__('%1$s database update required','woocommerce-product-options' ), $this->plugin->get_name()), 'message' => sprintf(__('<p>%1$s has been updated! To keep things running smoothly, we have to update your database to the newest version. The database update process runs in the background and may take a little while, so please be patient.</p>','woocommerce-product-options' ), $this->plugin->get_name()), 'buttons' => ['update-db' => ['value' => 'Update Database', 'href' => '#', 'class' => 'button-primary'], 'learn-more' => ['value' => 'Learn more about updates', 'href' => 'https://barn2.com/kb/learn-more-about-updates/', 'target' => '_blank', 'class' => 'button-secondary', 'style' => 'margin-left: 8px;']]], 'update_db_complete_notice' => ['title' => sprintf(__('%1$s database update done','woocommerce-product-options' ), $this->plugin->get_name()), 'message' => sprintf(__('<p>%1$s database update complete. Thank you for updating to the latest version!</p>','woocommerce-product-options' ), $this->plugin->get_name())]];
    }
    /**
    ...
    {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to perform this action.','woocommerce-product-options' ));
        }
        // If already locked, stop.
        if ($this->is_upgrade_locked()) {
            wp_die(__('An upgrade is already in progress. Please wait until it completes.','woocommerce-product-options' ));
        }
