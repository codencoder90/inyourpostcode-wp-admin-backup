<?php
/**
 * Admin Add Artwork Page
 * 
 * Purpose: Add new artwork form
 * نیا artwork add کرنے کا form
 * 
 * @package CustomizeDesignByTechXanders
 * @version 1.0
 */

if (!defined('ABSPATH')) {
    exit;
}

// Handle form submission
if (isset($_POST['cdbt_add_artwork']) && check_admin_referer('cdbt_add_artwork_action', 'cdbt_add_artwork_nonce')) {
    
    if (empty($_POST['artwork_title'])) {
        $error = __('Please enter artwork title.', CDBT_TEXT_DOMAIN);
    } elseif (empty($_FILES['artwork_image']['name'])) {
        $error = __('Please select an image file.', CDBT_TEXT_DOMAIN);
    } else {
        $handler = new CDBT_Artwork_Handler();
        $result = $handler->add_artwork(
            array(
                'title' => $_POST['artwork_title'],
                'display_order' => isset($_POST['display_order']) ? $_POST['display_order'] : 0
            ),
            $_FILES['artwork_image']
        );
        
        if ($result['success']) {
            $success = $result['message'];
            // Redirect to list page after 2 seconds
            echo '<script>
                setTimeout(function() {
                    window.location.href = "' . admin_url('admin.php?page=cdbt-artworks') . '";
                }, 2000);
            </script>';
        } else {
            $error = $result['message'];
        }
    }
}
?>

<div class="wrap cdbt-artwork-admin">
    <h1><?php _e('Add New Artwork', CDBT_TEXT_DOMAIN); ?></h1>
    
    <?php if (isset($success)): ?>
        <div class="notice notice-success is-dismissible">
            <p><?php echo $success; ?></p>
            <p><?php _e('Redirecting to artwork list...', CDBT_TEXT_DOMAIN); ?></p>
        </div>
    <?php endif; ?>
    
    <?php if (isset($error)): ?>
        <div class="notice notice-error is-dismissible">
            <p><?php echo $error; ?></p>
        </div>
    <?php endif; ?>
    
    <form method="post" enctype="multipart/form-data" class="cdbt-artwork-form">
        <?php wp_nonce_field('cdbt_add_artwork_action', 'cdbt_add_artwork_nonce'); ?>
        
        <table class="form-table">
            <tr>
                <th scope="row">
                    <label for="artwork_title">
                        <?php _e('Artwork Title', CDBT_TEXT_DOMAIN); ?> *
                    </label>
                </th>
                <td>
                    <input type="text" 
                           name="artwork_title" 
                           id="artwork_title" 
                           class="regular-text" 
                           required
                           value="<?php echo isset($_POST['artwork_title']) ? esc_attr($_POST['artwork_title']) : ''; ?>">
                    <p class="description">
                        <?php _e('Enter a descriptive title for this artwork', CDBT_TEXT_DOMAIN); ?>
                    </p>
                </td>
            </tr>
            
            <tr>
                <th scope="row">
                    <label for="artwork_image">
                        <?php _e('Artwork Image', CDBT_TEXT_DOMAIN); ?> *
                    </label>
                </th>
                <td>
                    <input type="file" 
                           name="artwork_image" 
                           id="artwork_image" 
                           accept="image/jpeg,image/jpg,image/png,image/gif"
                           required>
                    <p class="description">
                        <?php _e('Allowed formats: JPG, PNG, GIF. Maximum file size: 5MB', CDBT_TEXT_DOMAIN); ?>
                    </p>
                    
                    <!-- Image preview -->
                    <div id="image-preview" style="margin-top: 15px; display: none;">
                        <img id="preview-img" src="" alt="Preview" style="max-width: 300px; max-height: 300px; border: 1px solid #ddd; padding: 5px;">
                    </div>
                </td>
            </tr>
            
            <tr>
                <th scope="row">
                    <label for="display_order">
                        <?php _e('Display Order', CDBT_TEXT_DOMAIN); ?>
                    </label>
                </th>
                <td>
                    <input type="number" 
                           name="display_order" 
                           id="display_order" 
                           class="small-text" 
                           value="0"
                           min="0">
                    <p class="description">
                        <?php _e('Lower numbers appear first. Default is 0.', CDBT_TEXT_DOMAIN); ?>
                    </p>
                </td>
            </tr>
        </table>
        
        <p class="submit">
            <input type="submit" 
                   name="cdbt_add_artwork" 
                   class="button button-primary" 
                   value="<?php _e('Add Artwork', CDBT_TEXT_DOMAIN); ?>">
            <a href="<?php echo admin_url('admin.php?page=cdbt-artworks'); ?>" 
               class="button">
                <?php _e('Cancel', CDBT_TEXT_DOMAIN); ?>
            </a>
        </p>
    </form>
</div>

<script>
jQuery(document).ready(function($) {
    // Image preview
    $('#artwork_image').on('change', function(e) {
        var file = e.target.files[0];
        if (file) {
            var reader = new FileReader();
            reader.onload = function(e) {
                $('#preview-img').attr('src', e.target.result);
                $('#image-preview').show();
            };
            reader.readAsDataURL(file);
        } else {
            $('#image-preview').hide();
        }
    });
    
    // Form validation
    $('form.cdbt-artwork-form').on('submit', function(e) {
        var title = $('#artwork_title').val().trim();
        var file = $('#artwork_image')[0].files[0];
        
        if (!title) {
            alert('<?php _e('Please enter artwork title.', CDBT_TEXT_DOMAIN); ?>');
            e.preventDefault();
            return false;
        }
        
        if (!file) {
            alert('<?php _e('Please select an image file.', CDBT_TEXT_DOMAIN); ?>');
            e.preventDefault();
            return false;
        }
        
        // Check file size
        var maxSize = 5 * 1024 * 1024; // 5MB
        if (file.size > maxSize) {
            alert('<?php _e('File size too large. Maximum 5MB allowed.', CDBT_TEXT_DOMAIN); ?>');
            e.preventDefault();
            return false;
        }
        
        // Check file type
        var allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
        if (allowedTypes.indexOf(file.type) === -1) {
            alert('<?php _e('Invalid file type. Only JPG, PNG, GIF allowed.', CDBT_TEXT_DOMAIN); ?>');
            e.preventDefault();
            return false;
        }
        
        return true;
    });
});
</script>

<style>
.cdbt-artwork-form {
    background: #fff;
    padding: 20px;
    margin-top: 20px;
    border: 1px solid #c3c4c7;
}

.cdbt-artwork-form .form-table th {
    width: 200px;
}

#image-preview {
    background: #f6f7f7;
    padding: 10px;
    border-radius: 4px;
}

#preview-img {
    border-radius: 4px;
}
</style>
