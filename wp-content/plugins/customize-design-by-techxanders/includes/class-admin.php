...
class CDBT_Admin {
    ...
    public function __construct() {
        ...
        add_action('admin_menu', array($this, 'register_artwork_menu'));
        add_action('admin_init', array($this, 'register_settings'));
    }
    
    
    public function register_artwork_menu() {
    // Main menu
    add_menu_page(
        'Manage Artworks',                    // Page title
        'Artworks',                           // Menu title  
        'manage_options',                     // Capability
        'cdbt-artworks',                      // Menu slug
        array($this, 'artwork_list_page'),   // Callback
        'dashicons-images-alt2',             // Icon
        30                                    // Position
    );
    
    // Submenu: All Artworks
    add_submenu_page(
        'cdbt-artworks',
        'All Artworks',
        'All Artworks',
    ...
    add_submenu_page(
        ...
        'manage_options',
        'cdbt-artwork-edit',
        array($this, 'artwork_edit_page')
    );
}

/**
 * Artwork list page callback
 */
public function artwork_list_page() {
    require_once plugin_dir_path(__FILE__) . '../admin/artwork-list.php';
}

/**
 * Artwork add page callback
 */
public function artwork_add_page() {
    require_once plugin_dir_path(__FILE__) . '../admin/artwork-add.php';
}

/**
 * Artwork edit page callback
 */
public function artwork_edit_page() {
    require_once plugin_dir_path(__FILE__) . '../admin/artwork-edit.php';
}
    ...
    public function add_product_data_panel() {
                    ...
                    <div id="cdbt_frame_sizes">
                        <?php if (!empty($frame_sizes)): ?>
                            <?php foreach ($frame_sizes as $index => $frame): ?>
                                <div class="cdbt_frame_size_row" data-index="<?php echo $index; ?>" style="padding:15px; border:1px solid #ddd; margin-bottom:15px; background:#f9f9f9; clear:both;">
                                    <div style="margin-bottom:15px; clear:both;">
                                        <div style="display:block; font-weight:700; font-size:14px; color:#1d2327; margin-bottom:8px; float:none !important; width:auto !important;"><?php _e('Frame Size', CDBT_TEXT_DOMAIN); ?></div>
                                        <input type="text" style="width:100% !important; max-width:400px !important; display:block !important;" name="_cdbt_frame_sizes[<?php echo $index; ?>][size]" value="<?php echo esc_attr($frame['size']); ?>" placeholder="<?php _e('e.g., 8x10 inches', CDBT_TEXT_DOMAIN); ?>" />
                                    </div>
                                    <div style="margin-bottom:15px; clear:both;">
                                        <div style="display:block; font-weight:700; font-size:14px; color:#1d2327; margin-bottom:8px; float:none !important; width:auto !important;"><?php _e('Frame Price', CDBT_TEXT_DOMAIN); ?></div>
                                        <input type="number" style="width:100% !important; max-width:200px !important; display:block !important;" step="0.01" name="_cdbt_frame_sizes[<?php echo $index; ?>][price]" value="<?php echo esc_attr($frame['price']); ?>" placeholder="0.00" />
                                    </div>
                                    <div style="clear:both; margin-top:10px; display:inline-block;     width: 100%;">
        ...
        <script type="text/template" id="cdbt_frame_size_template">
            <div class="cdbt_frame_size_row" data-index="{{index}}" style="padding:15px; border:1px solid #ddd; margin-bottom:15px; background:#f9f9f9; clear:both;">
                <div style="margin-bottom:15px; clear:both;">
                    <div style="display:block; font-weight:700; font-size:14px; color:#1d2327; margin-bottom:8px; float:none !important; width:auto !important;"><?php _e('Frame Size', CDBT_TEXT_DOMAIN); ?></div>
                    <input type="text" style="width:100% !important; max-width:400px !important; display:block !important;" name="_cdbt_frame_sizes[{{index}}][size]" value="" placeholder="<?php _e('e.g., 8x10 inches', CDBT_TEXT_DOMAIN); ?>" />
                </div>
                <div style="margin-bottom:15px; clear:both;">
                    <div style="display:block; font-weight:700; font-size:14px; color:#1d2327; margin-bottom:8px; float:none !important; width:auto !important;"><?php _e('Frame Price', CDBT_TEXT_DOMAIN); ?></div>
                    <input type="number" style="width:100% !important; max-width:200px !important; display:block !important;" step="0.01" name="_cdbt_frame_sizes[{{index}}][price]" value="" placeholder="0.00" />
                </div>
                <div style="clear:both; margin-top:10px;     width: 100%; display:inline-block">
                    <button type="button" class="button cdbt_remove_frame_size"><?php _e('Remove', CDBT_TEXT_DOMAIN); ?></button>
                </div>
            </div>
        </script>
        ...
        jQuery(document).ready(function($) {
            ...
            var addingFrame = false;
            
            // Unbind any existing handlers first
            $('#cdbt_add_frame_size').off('click');
            
            // Use direct binding with prevention flag
            $('#cdbt_add_frame_size').on('click', function(e) {
                e.preventDefault();
    ...
    public function save_product_data($post_id) {
        ...
        // NEW: Color photos (with normalized slugs)
        if (isset($_POST['_cdbt_color_photos']) && is_array($_POST['_cdbt_color_photos'])) {
            ...
            foreach ($_POST['_cdbt_color_photos'] as $color => $photos) {
                ...
                $color_photos[$normalized_color] = array(
